...
type Status = 'Connected' | 'Idle' | 'Needs attention' | 'Offline'
type RadioRow = {
  id: string
  name: string
  operator: string
...
const radios: RadioRow[] = [
  { id: 'RADIO-001', name: 'Alpha Unit', operator: 'Ahmed Khalil', initials: 'AK', group: 'Operations', status: 'Connected', channel: 'CH-01', frequency: 'Admin configured', lastSeen: 'Now', config: 'v1.4', tone: 'CTCSS 88.5' },
  { id: 'RADIO-002', name: 'Bravo Unit', operator: 'Sara Nasser', initials: 'SN', group: 'Security', status: 'Connected', channel: 'CH-01', frequency: 'Admin configured', lastSeen: 'Now', config: 'v1.4', tone: 'CTCSS 88.5' },
  { id: 'RADIO-003', name: 'Charlie Unit', operator: 'Omar Youssef', initials: 'OY', group: 'Maintenance', status: 'Idle', channel: 'CH-02', frequency: 'Admin configured', lastSeen: '8m ago', config: 'v1.3', tone: 'DCS 023' },
  { id: 'RADIO-004', name: 'Delta Unit', operator: 'Noor Hamad', initials: 'NH', group: 'Operations', status: 'Needs attention', channel: 'CH-03', frequency: 'Admin configured', lastSeen: '26m ago', config: 'v1.2', tone: 'Not configured' },
  { id: 'RADIO-005', name: 'Echo Unit', operator: 'Unassigned', initials: '—', group: 'Unassigned', status: 'Offline', channel: 'CH-01', frequency: 'Admin configured', lastSeen: '2h ago', config: 'v1.1', tone: 'CTCSS 88.5' },
]
...
function StatusPill({ status }: { status: Status }) {
  const tone = status === 'Connected' ? 'success' : status === 'Offline' ? 'danger' : status === 'Idle' ? 'warning' : 'attention'
  ...
  return <div className="modal-backdrop" onClick={onClose}>
    <section className="modal" onClick={(e) => e.stopPropagation()} role="dialog" aria-modal="true" aria-labelledby="add-radio-title">
      ...
      <div className="modal-body">
        ...
        {step === 3 && <><div className="form-grid"><label>Radio ID<input placeholder="RADIO-006" /></label><label>Device name<input placeholder="Enter a name" /></label><label>Assigned user<input placeholder="Unassigned" /></label><label>Group<select><option>Select group</option><option>Operations</option><option>Security</option></select></label><label>Licensed channel<input placeholder="CH-01" /></label><label>Licensed frequency<input placeholder="Enter licensed frequency" /></label><label>CTCSS / DCS<input placeholder="Optional" /></label><label>Power setting<select><option>Admin selects after validation</option></select></label></div><div className="license-note"><AlertTriangle size={16} /><span>Frequencies must be entered by an authorized administrator according to the applicable license or permit. No frequency is pre-filled.</span></div></>}
